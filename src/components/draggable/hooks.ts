...
export function initDraggableContainer(
  ...
  const _unselect = (e: HandleEvent) => {
    const draggable = (e.target as HTMLButtonElement).closest('.draggable')
    // debugger
    const target = e.target
    const leftDom = (e.target as HTMLButtonElement).closest('.left_main')
    const rightDom = (e.target as HTMLButtonElement).closest('.right_main')
    const colorDom = (e.target as HTMLButtonElement).closest('.el-color-picker__panel')
    const selectDom = (e.target as HTMLButtonElement).closest('.el-select-dropdown')
    if (!containerRef.value?.contains(<Node>target) && !leftDom && !rightDom && !colorDom && !selectDom) {
      setEnable(false)
      setDragging(false)
      setResizing(false)
      setResizingHandle('')
  ...
  const handleUp = () => {
    /**
     * @name conflict
     * @param (x/顶部距离,y/左侧距离,w/宽,h/高,nodes/父元素实例)
    ...
    const isConflict = conflict(
      ...
      w.value,
      h.value,
      containerRef.value!.parentNode!.childNodes,
      containerRef.value!
    )
  ...
  onMounted(() => {
    ...
    el.style.left = x + 'px'
    el.style.top = y + 'px'
    document.documentElement.addEventListener('mousedown', _unselect,true)
